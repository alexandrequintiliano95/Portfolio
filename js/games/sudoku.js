...
(function () {
    ...
    function createSudoku(root) {
        ...
        let startedAt = 0;
        let ticker = 0;
        // Dentro do modal o jogo só responde enquanto ele está aberto
        let visible = !modal;
        ...
        document.addEventListener('keydown', event => {
            // com o modal fechado o teclado pertence à página, não ao jogo
            if (!visible) return;
        ...
        if (modal) {
            modal.addEventListener('modal:open', () => {
                visible = true;
                showBest();
                // retoma o cronômetro de uma partida já começada
                if (!solved && elapsed > 0) startTimer();
            });
